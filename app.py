...
def save_data(rms_dept, manager_name, results):
    """Supabase DB 저장 (PostgreSQL Upsert 적용 - 작성자 컬럼 추가)"""
...
# --- 메인 UI ---
def main():
    st.set_page_config(page_title="KB증권 대외계-RMS 분리 작업 관련 대시보드", layout="wide")
    ...
    if mapping:
        ...
        with col1:
            ...
            manager_name = st.text_input("👤 작성자", value=default_manager, key=f"manager_{selected_rms}")
